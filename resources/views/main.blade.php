<main class="container mx-auto p-6 md:p-12">
    ...
    <section id="about" class="mb-12 md:flex md:space-x-8">
        <div class="md:w-1/3">
            <img src="{!! Storage::disk('liara')->url($about->image) !!}" alt="Your Name" class="w-full rounded-full shadow-lg">
        </div>
            ...
            <div class="space-y-6">
                <div class="bg-white p-6 rounded-lg shadow-lg">
                    ...
                    @if(app()->currentLocale() === 'fa')
                        ...
                        @endphp
                        <p class="text-gray-600">{{ $persianFormatter->format($from) . ' تا ' . $persianFormatter->format($to) }}</p>
                    @else
                        <p class="text-gray-600">{{ $experience->from . ' to ' . $experience->to }}</p>
                    @endif
{{--                    <p class="mt-2">Brief description of your role and achievements. Highlight your key responsibilities and successes.</p>--}}
    ...
    {{--                    <p>Description or proficiency level.</p>--}}
                        <div class="relative pt-1">
                            ...
                            <div class="flex bg-gray-200 rounded-full h-3">
                                <div class="bg-blue-600 h-full rounded-full" style="width: {!! (int)$skill->proficiency !!}%;"></div>
                            </div>
                        </div>
                    </div>
                @endforeach
                ...
                @foreach($portfolio as $project)
                    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
                        ...
                        <div class="p-6">
                            ...
                            <p class="text-gray-600 mb-4">{{ json_decode($project->description)->{app()->currentLocale()} }}</p>
    ...
    @if($blog_posts->count() > 0)
        <section id="blog" class="mb-12">
            <h2 class="mb-8"><a href="{{ route('blog.home') }}" class="text-3xl font-semibold hover:underline hover:text-blue-500">{{ __('mainsections.latestposts') }}</a></h2>
            <div class="space-y-6">
                @forelse($blog_posts as $post)
                    <div class="bg-white p-6 rounded-lg shadow-lg">
                        <h3 class="text-xl font-semibold mb-2">
                            <a href="{{ route('blog.post.show', ['slug' => $post->slug]) }}" class="text-blue-600 hover:underline">
    ...
    @else
        <section id="blog" class="mb-12">
            <h2><a href="{{ route('blog.home') }}" class="text-3xl font-semibold hover:text-blue-500">{{ __('mainsections.latestposts') }}</a></h2>
        </section>
    @endif
            ...
            <div class="md:w-1/2 mb-8 md:mb-0">
                @foreach($contact_info as $ci)
                    @if(strtolower($ci->platform) == 'email')
                        <p class="text-xl mb-4"><i class="mx-4 {{ $ci->icon }}"></i> <a href="mailto:{!! $ci->url !!}" class="text-blue-600 hover:underline">{!! $ci->url !!}</a></p>
                    @else
                        <p class="text-xl mb-4"><i class="mx-4 {{ $ci->icon }}"></i> <a href="{!! $ci->url !!}" class="text-blue-600 hover:underline capitalize">{!! $ci->platform !!}</a></p>
                    @endif
                @endforeach
            </div>

            <!-- Contact Form -->
            <div class="md:w-1/2 bg-white p-8 rounded-lg shadow-lg w-full">
                <h2 class="text-2xl font-bold text-gray-800 text-center mb-6">{{ __('mainsections.contact_me_title') }}</h2>
                <form action="{{ route('contact-form.store') }}" method="POST">
                    ...
                    <!-- Name -->
                    <div class="mb-4">
                        <label for="name" class="block text-gray-700 font-medium mb-2">{{ __('mainsections.contact_me_name') }}</label>
                    ...
                    <!-- Email -->
                    <div class="mb-4">
                        <label for="email" class="block text-gray-700 font-medium mb-2">{{ __('mainsections.contact_me_email') }}</label>
                    ...
                    <!-- Subject -->
                    <div class="mb-4">
                        <label for="subject" class="block text-gray-700 font-medium mb-2">{{ __('mainsections.contact_me_subject') }}</label>
                    ...
                    <!-- Message -->
                    <div class="mb-6">
                        <label for="message" class="block text-gray-700 font-medium mb-2">{{ __('mainsections.contact_me_message') }}</label>
                        <textarea id="message" name="message" rows="5" {{ old('message') }}
                                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none" required></textarea>
                    </div>
                    @error('message')
                        <span class="text-red-500">{{ $message }}</span>
                    @enderror
                    ...
                    <div class="text-center">
                        <button type="submit" class="w-full bg-blue-500 text-white font-medium py-2 px-4 rounded-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
